...
namespace Web.BackEnd.Controllers
{
    public class GameController : BackEndController
    {
        ...
        #region Add Exp
        string CheckAddExpValid(AddExpModel addExpModel)
        {
            string message = "";

            if (string.IsNullOrEmpty(addExpModel.AccountName))
            {
                message += "Nhập tài khoản<br/>";
            }

            if (addExpModel.SelectedServerId <= 0)
            {
                message += "Chọn server<br/>";
            }

            if (addExpModel.SelectedRoleId <= 0)
            {
                message += "Chọn nhân vật<br/>";
            }

            if (addExpModel.Exp <= 0)
            {
                message += "Kinh nghiệm không hợp lệ<br/>";
            }

            message += addExpModel.CheckCaptcha();
        ...
        [HttpPost]
        public async Task<JsonResult> AddExp(AddExpModel addExpModel)
        {
            MessageModel messageModel = new MessageModel();

            string message = CheckAddExpValid(addExpModel);
            ...
            {
                int accountId = userRepository.GetAccountId(addExpModel.AccountName);
                if (accountId > 0)
                {
                    AddExpRequest addExpRequest = new AddExpRequest()
                    {
                        UserId = 1,
                        ServerId = addExpModel.SelectedServerId,
                        RoleId = addExpModel.SelectedRoleId,
                        Exp = addExpModel.Exp
                    };
                    ...
                    {
                        messageModel.Message = "Thêm kinh nghiệm thành công";
                        messageModel.Flag = true;

                        BackEndUtilities.WriteLog((int)Enums.BackEndFunction.AddExp, messageModel.Message, 1);
                    }
                    else
                    {
                        messageModel.Message = "Thêm kinh nghiệm thất bại";
                        messageModel.Flag = false;

                        BackEndUtilities.WriteLog((int)Enums.BackEndFunction.AddExp, messageModel.Message, 0);
                    }
                }
            }
        ...
        public async Task<JsonResult> SendMail(SendMailModel sendMailModel)
        {
            ...
            else
            {
                ...
                if (accountId > 0)
                {
                    ...
                    string logs = "Gửi thư thành công" +
                                " | Account:" + sendMailModel.AccountName +
                                " | ServerId :" + sendMailModel.SelectedServerId +
